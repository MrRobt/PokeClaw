...
class CloudDeviceTokenStoreTest {
    ...
    @Test
    fun `saveDeviceToken 兼容方法在有历史记录时保留原 refreshToken`() {
        // 先完成首次注册
        tokenStore.saveTokens(
            deviceToken = "device_token_123",
            refreshToken = "refresh_token_456",
        ...
        // 使用兼容方法只更新 deviceToken
        tokenStore.saveDeviceToken(
            token = "new_device_token_789",
    ...
    @Test
    fun `saveDeviceToken 兼容方法首次调用应抛出异常`() {
        // 首次调用 saveDeviceToken（无历史记录）
        val exception = assertThrows(IllegalStateException::class.java) {
            tokenStore.saveDeviceToken(
        ...
        assertTrue("错误信息应提示首次必须使用 saveTokens()", exception.message?.contains("saveTokens") == true)
    }

    @Test
    fun `saveRefreshToken 兼容方法在有历史记录时保留原 deviceToken`() {
        // 先完成首次注册
        tokenStore.saveTokens(
            deviceToken = "device_token_123",
            refreshToken = "refresh_token_456",
        ...
        // 使用兼容方法只更新 refreshToken
        tokenStore.saveRefreshToken("new_refresh_token_abc")

        // 验证 refreshToken 已更新，deviceToken 保持不变
        val snapshot = tokenStore.snapshot()
        assertEquals("deviceToken 应保持不变", "device_token_123", snapshot?.deviceToken)
        assertEquals("refreshToken 应更新", "new_refresh_token_abc", snapshot?.refreshToken)
    }

    @Test
    fun `saveRefreshToken 兼容方法首次调用应抛出异常`() {
        // 首次调用 saveRefreshToken（无历史记录）
        val exception = assertThrows(IllegalStateException::class.java) {
            tokenStore.saveRefreshToken("refresh_token_456")
        }
        assertTrue("错误信息应提示首次必须使用 saveTokens()", exception.message?.contains("saveTokens") == true)
    }
