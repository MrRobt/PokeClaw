...
| 日期 | 状态 | 问题编号 | 描述 |
|------|------|----------|------|
| 2026-07-09 | PARTIAL(源码核实) | H6-CLOUD-AGENT-FALLBACK + REMOTE-SKILL | **P0-1 云任务 agent-loop 兜底 + P0-2 远程下发 skill 安装**(采纳并核实此前 codex 遗留的未提交端侧代码)。P0-1:`LocalAgentTaskExecutor` map 不到确定性 skill(conf<0.5)时不再 `TASK_REJECTED`,转完整 MiniMax agent-loop(经 `AppViewModel.startTask`,`suspendCancellableCoroutine` + `AtomicBoolean` 幂等 resume + 超时 `stopTask` 兜底 + 取消停任务)= 端侧「技能→完整 AI」升级层。P0-2:`install_skill:<JSON>` 设备任务通道 + `RemoteSkillInstaller`(marketplace 同步 / 解析 steps / `SkillRegistry.register` / 回执)+ `DebugTaskReceiver.skill_action=sync` ADB 触发。附带修 `BuiltInSkills` 两坏工具名(`launch_app`→`open_app`+`package_name`、`screenshot`→`take_screenshot`,已对照真实工具定义)。**核实**:全部引用符号解析通过 + 并发路径源码审查通过(双 resume CAS 防护 / 超时清理 / 取消停任务)+ 工具名正确。**⚠️ device-runtime E3 阻塞**:无 Android 编译环境(本地 OOM / 测试服无 SDK / emulator 旧包 v0.7.2);ADB E2E 已设计(`am broadcast … --es skill_action sync` 验 SkillRegistry 注入;下发 `install_skill:` + 非 skill 复合任务 验兜底 round),待编译环境执行(BACKLOG P1)。|
