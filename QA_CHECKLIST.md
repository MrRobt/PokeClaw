...
| 日期 | 状态 | 问题编号 | 描述 |
|------|------|----------|------|
| 2026-07-09 | 代码完成(未编译) | H7-CROSS-DEVICE-EVOLUTION | **跨设备自进化上报闭合**(报告 row 9 此前 STUB:`ExperienceUploader` 从不被调用、自进化只在本地不回传)。dyq 新增设备端点 `POST /api/claw-device/experience`(复用设备鉴权 interceptor + customizer 放行)→ 写 `claw_experience`(sourceDeviceId 维度:executionType/summary/rewardScore/rewardDetail)。端侧 `ExperienceUploader` URL 对齐 + `CloudNodeOrchestrator.reportExperience` 在云任务终态上报(success→uploadSuccess / fail→uploadFailure,best-effort 不阻塞结果上报)。⚠️ **用户要求不编译直接提交**:未编译 / 未装包端到端真验收(待补:装包→跑云任务→查 `claw_experience` 新行 sourceDeviceId=本机)。|
| 2026-07-09 | PASS(mvn编译) | H3-二级-CLOUD-AGENT | **H3 三级降级链·第二级「云端大模型接手」闭合**:此前 executorType=cloud_agent 任务经 `dispatchToCloud` 落 `claw_task`(PENDING)后**无人执行**(有分派、无执行者)。新增独立 cloud-agent MQ 链路(`ClawCloudAgentTaskMessage` / `ClawCloudAgentRabbitMQConfig` 独立队列+DLQ / `ClawCloudAgentTaskConsumer`)+ `dispatchToCloud` 落库后投递 → 消费者用 **cloud_agent 独立身份**(`executorUserId`,**不混真实用户** userId,Nicole 要求;agent 小龙虾/策略/经验独立积累)调 `Hermes.command` → `completeTask/failTask` 回写。**mvn -pl dyq-module-claw-biz -am compile BUILD SUCCESS**。至此三级链全闭合:①端侧技能→AgentLoop(device-E3)②云端 Hermes 接手(本次 mvn)③人工接管告警(H3-A)。⚠️ 待 Hermes 环境端到端真验收。|
| 2026-07-09 | PASS(device-E3) | H6b-P0-DEVICE-E3 | **P0-1/P0-2 真机 device-E3 达成**(推翻"无编译环境":测试服 64GB RAM + `/root/android-sdk` 真编译 `assembleDirectDebug` **BUILD SUCCESSFUL**〔compile+dex+R8 全通过,含 P0 代码〕→ 装 emulator `localhost:5555` → ADB 注入 dyq 任务真跑）。**P0-2 远程下发 skill = 端到端 SUCCESS**:注入 `install_skill:<JSON>` 设备任务 → `onPendingTasksAvailable`→`executeCloudTask`→`execute: ✅ 已安装远程下发 skill id=remote_demo steps=2; SkillRegistry 13→14` → dyq 回传 **SUCCESS**。**P0-1 兜底 = PASS**:非-skill 复合任务 → `execute: 无确定性 skill 映射,转 agent-loop 兜底`+`onLoopStart round=1`(替代旧 `TASK_REJECTED`);本次 MiniMax loop round-1 未出终态,**240s 超时兜底正确触发** `runAgentLoopFallback: agent loop 超时未产生终态` → dyq **FAILED+retryable**(exec=240084ms)**无死锁/无卡死**。⚠️次要:端侧 agent loop 对 round-1 LLM 快速失败未走 onError、靠 240s 超时兜底(BACKLOG P2)。|
| 2026-07-09 | PARTIAL(源码核实,已被 H6b device-E3 取代) | H6-CLOUD-AGENT-FALLBACK + REMOTE-SKILL | **P0-1 云任务 agent-loop 兜底 + P0-2 远程下发 skill 安装**(采纳并核实此前 codex 遗留的未提交端侧代码)。P0-1:`LocalAgentTaskExecutor` map 不到确定性 skill(conf<0.5)时不再 `TASK_REJECTED`,转完整 MiniMax agent-loop(经 `AppViewModel.startTask`,`suspendCancellableCoroutine` + `AtomicBoolean` 幂等 resume + 超时 `stopTask` 兜底 + 取消停任务)= 端侧「技能→完整 AI」升级层。P0-2:`install_skill:<JSON>` 设备任务通道 + `RemoteSkillInstaller`(marketplace 同步 / 解析 steps / `SkillRegistry.register` / 回执)+ `DebugTaskReceiver.skill_action=sync` ADB 触发。附带修 `BuiltInSkills` 两坏工具名(`launch_app`→`open_app`+`package_name`、`screenshot`→`take_screenshot`,已对照真实工具定义)。**核实**:全部引用符号解析通过 + 并发路径源码审查通过(双 resume CAS 防护 / 超时清理 / 取消停任务)+ 工具名正确。**device-runtime E3 后续已达成(见 H6b 行)**:此前"阻塞"判断有误——测试服即有编译环境(64GB+`/root/android-sdk`),已真编译+装机+ADB 真跑通过。|
| 2026-07-09 | PASS | H5-YOLO-REAL-TRAIN | **数据飞轮训练环节真验收(E3,测试服务器 torch+ultralytics+gxe)**:android 真截图 → yolo_ui_v0 自蒸馏 38 弱标签 → ultralytics 真训 2 epoch(CPU）→ 产出 **22.5MB best.pt**(`tools/train_smoke.py`）。`training_sim._real_train` 从占位（硬编码 yolov8s 下载 + data.yaml 未组织标签）改为正确实现:`_build_ultralytics_dataset` 把 `raw/*.png`+`yolo_labels/*.txt` 组织成 ultralytics `images/labels` 布局(本地验证:仅含已标注样本、布局正确 **PASS**）,`_real_train` 环境可配(base/epochs/imgsz/device,CPU smoke 或 GPU full）。hub 现有 **5/5 e2e 仍通过**。|
| 2026-07-09 | PASS/PARTIAL | H3-DEGRADATION | **H3 降级链·第三级(人工接管)闭合**:补 `ClawTaskTakeoverConsumer`(@RabbitListener)接住接管通知——此前 `ClawTaskTakeoverMessage` fire-and-forget 无消费者、靠人轮询;现做告警级日志(可扩展工单/IM)。mvn 编译 BUILD SUCCESS。**端侧"技能→完整 AgentLoop"升级已有**(`LocalAgentTaskExecutor.runAgentLoopFallback`)。⚠️ **二级"云端大模型接手"受阻于架构 gap**:cloud_agent 任务全链路(`ClawTaskDispatchReqVO`/`ClawTaskDO`/`ClawTaskAssignedMessage`)**无 userId**,而 `ClawHermesService.command` 需 userId(每用户小龙虾)→ 需先决策 cloud_agent 如何绑执行主体(见 BACKLOG,不擅自贯穿改)。端云协议 needsEscalation 待补。|
| 2026-07-09 | PASS | H2-GATE + JAVA-COMPILE | **H2 安全上端门禁**:`ClawPolicyServiceImpl.createPolicyVersion` 改为 新版本先落 `CANDIDATE` → 晋升门禁(avgReward≥0 且 sampleCount≥10,借 claw-yolo-hub promote gate 范式)→ 达标才升 current,不达标保留 candidate 不上端(旧版本继续服务)。**本地 `mvn -pl dyq-module-claw-biz compile` BUILD SUCCESS(367 源文件)** —— H2 + 之前全部 YOLO Java(4 DO+4 Mapper+Service+Controller+VO+`ClawYoloAuthorizeRequestsCustomizer`)编译验证通过(CODE-READY→编译)。另 **H1 OCR 完整验证**:easyocr 装好后对 android(屏上跑着 **PokeClaw app**)真屏 OCR 19 文本框(conf 0.97-1.00),decision OCR 文本匹配「1.02」→ `tap(45,26)` 定位真跑通。⚠️ H2 运行 E2E(真训练回调触发门禁)需部署 dyq jar,测试环境具备条件(有 dyq 实例可重建)。|
| 2026-07-09 | PASS | H1-VISION-8081-E3 | ⭐ **感知大脑真跑通(E3)**——路线A(control 脚本群控)的视觉大脑,归 control 模块 `dyq-module-control/vision-engine`(方案①:Java 编排 + Python 引擎)。测试环境 `pokeclaw-test`:真 YOLO(ultralytics 8.4 + `yolo_ui_v0.pt`)对 android `emulator-5554` 真屏(720×1280)检测 **39 框/7.3s CPU**(16 icon+23 interactable,真坐标+置信 0.9)。HTTP 端到端:`/health` detector_available=true、`/api/yolo/detect` 返回真框、`/api/multimodal/decision` 返回**真 tap 坐标**(顶层扁平 target_x/y,OCR 缺则回退检测框、**诚实不造假**)、`/verify` 工作。契约本地 7/7。**替掉 control 原写死假数据**(action=click/0.95)。`selfcheck.py` 一键复验。⚠️ easyocr 未装→OCR 降级(decision 退检测框回退,仍真坐标);装 easyocr 后可文本匹配。|
| 2026-07-09 | PASS | H4-ENDCLOUD-E2E-E3 | ⭐ **端云链路真后端 E2E(E3)**——测试环境 `pokeclaw-test`(192.168.250.3)docker `dyq-server-full-test` `:48081`（mysql+redis+rabbitmq 齐）。脚本 `dyq-module-claw/scripts/verify_device_e2e.py`：register→HTTP200 code=0 拿 deviceToken；heartbeat→200；pending-tasks→200；**submitResult HMAC 签名通过 filter 进业务层**（signing_string=ts\n+nonce\n+path\n+sha256(body)，HMAC-SHA256(deviceToken)，端云一致）；**坏签名对照→401 code=401001 被拒**（HMAC 真校验非摆设）。**证伪 `CLOUD_SUBSYSTEM_BOUNDARY.md` 的「mock-only / E1 / 从未真后端验证」**——端云链路真通。⚠️ 附带缺陷记 BACKLOG：`submitTaskResult` 对不存在 taskUuid 抛异常被全局兜底成 **HTTP 200 code=500「系统异常」**，应返回明确「任务不存在」而非 500。|
| 2026-07-08 | CODE-READY | AGENT-CLAW-YOLO-JAVA+INTEGRATION | 迭代（用户反馈：云端改用 dyq Java claw 模块实现、Python 作 mock、保持单图标、全面测试）。**单图标**：`VisionConsoleActivity` 去掉 LAUNCHER intent-filter，保留 `exported=true`（`adb am start` 打开）。**App 集成测试（E1，待测试服务器跑）**：新增 4 个 MockWebServer 集成测试——`ModelHubClientTest`（resolve/download/upload/train/promote/rollback/list 的 path+body+解析）、`CloudPhoneClientTest`（CommonResult 解封+control body+auth/tenant 头）、`YoloModelClientIntegrationTest`（resolve→下载→缓存→版本更新/校验失败/任务中延迟）、`ExplorerEndToEndTest`（fake 云手机状态机 + 真 hub 客户端 → 探索→采集→按 software_key 上传，断言每新页 1 次上传、去重）。`ModelHubClient` 增强：兼容 `CommonResult{code,data}` 信封与裸 JSON、支持相对 `download_url`（一套客户端连 Python mock 或 Java 云端皆可）。**Java 云端实现（CODE-READY，本机内存不足未编译）**：`dyq-module-claw` 新增 YOLO 模型中心（4 DO + 4 Mapper + `YoloHubService[Impl]` + `YoloDeviceController` @`/api/v1` @PermitAll + Req/Resp VO snake_case + `ClawYoloAuthorizeRequestsCustomizer` + 建表 SQL `V20260708__claw_yolo_tables.sql`），照 `ClawTraining*` 模板，逻辑等价 Python hub（路由/模拟训练/门禁/回滚/懒种子）。Python `claw-yolo-hub` 保留为 mock/参考（pytest 5/5 仍绿）。**编译验证在测试服务器**：Java `mvn -o -pl dyq-module-claw/dyq-module-claw-biz -DskipTests compile`；App `gradlew :app:testDirectDebugUnitTest --tests "io.agents.pokeclaw.{cloud,vision,explore,collect}.*"`。见 `docs/AGENT_CLAW_YOLO_SYSTEM.md`。|
| 2026-07-08 | PASS | AGENT-CLAW-YOLO-BUILD+HUB | ⭐ **App-side Agent→Claw + 云端 per-software YOLO 系统**（见 `docs/AGENT_CLAW_YOLO_SYSTEM.md` / 新 section Y）。**App 侧 E1（unit）**：新增 `vision/`+`explore/`+`collect/`+`device/`+`cloud/modelhub`+`cloud/cloudphone`+`ui/console/VisionConsoleActivity`；纯逻辑 19 条 JVM 单测全绿（WeakLabelDetector 4 / ModelRouting 4 / ModelCache 3 / StateHasher 3 / ExplorationEngine 2 / CollectPayload 2 / DataCollector 1）——`:app:testDirectDebugUnitTest --tests "io.agents.pokeclaw.{vision,explore,collect}.*"` BUILD SUCCESSFUL；**全量 `:app:assembleDirectDebug` 出 APK**（含 UI + manifest 合并）。**云端 RUNTIME-VERIFIED**：`../dyq/claw-yolo-hub`（FastAPI，仅 fastapi+stdlib）实起服务，`pytest` 5/5 走 collect→train→promote→rollback；live curl `/healthz`（seed 4 software）、TikTok→category、Claw→generic 兜底并**下发真 22MB gxe `yolo_ui_v0.pt`**（checksum 校验一致）。**E3（真机整链路）= BLOCKED**：本沙箱无 adb/设备，`SoftwareExplorer`/`Accessibility*`/`CloudPhoneClient` 未在真云手机跑通（`YoloModelBackend` 仍以 weak-label 代替真 ONNX 推理，训练为 simulated）。|
| 2026-07-08 | PASS | MINIMAX-ANTHROPIC-E3 | ⭐ **MiniMax（Anthropic 协议端点）真机 E3——零代码改动接上**。`https://api.minimaxi.com/anthropic/v1` 实测 **Bearer 与 x-api-key 都通**，PokeClaw 现有 `AnthropicLlmClient`（x-api-key + `config.baseUrl`）直接可用，无需改代码/重编。#2（Reno 10 Pro+）配 `provider=ANTHROPIC model=MiniMax-M3` → 任务 `open chrome and search for bitcoin price` → agent loop **8 轮**自主完成（`open_app→tap_node→get_screen_info→input_text→system_key→tap_node→get_screen_info→finish`），操作真 Chrome 取到 Bitcoin 价格。`onComplete rounds=8 totalTokens=15737 model=MiniMax-M3` + auto-return。reliability 全程：`finishTask events=26 actions=8 failed=0 validationFailed=0`。**MiniMax 比 DeepSeek 更省**（8 轮/15.7k tokens vs 12 轮/115.9k）。⚠️ 模型有视觉但当前 agent 只发无障碍文本、不发截图 → 真·多模态需另加「截图发 LLM」（未做）。|
| 2026-07-08 | PASS | CORE-AGENTLOOP-E3-CLOUDPHONE | ⭐⭐ **核心功能真机 E3**（#2 Reno 10 Pro+ PHU110 / Android 15 / arm64 / 加州；DeepSeek `deepseek-chat`）。任务 `open chrome and search for weather` → **agent loop 12 轮自主完成**：`input_text "weather"` → `system_key enter` → `tap_node` → `get_screen_info` → `wait` → `tap_node` → `swipe` → `finish`。**操作真实 Chrome、提取真实天气**：`Los Angeles County CA: 66°F Clear, Heat Advisory, 湿度76% 风3mph AQI47`（地点与 #2 加州一致=真实读屏）。`onComplete rounds=12 totalTokens=115862 model=deepseek-v4-flash` + auto-return chatroom。**reliability 全程护航**：`finishTask events=38 actions=12 failed=0 validationFailed=0`（12 个工具动作全走 ActionValidator+ExecutionTrace）。核心「AI 自主操作手机完成跨 app 任务」闭环在真 arm64 设备端到端通过。装机用「手机 curl 下载 catbox + 本地 pm install」把 45min 降到 <2min（adb 链路 ~58KB/s 慢，绕过）。|
| 2026-07-08 | PASS | DIRECT-TOOLS-E3 + RELIABILITY-VALIDATION-E3 | 真机 E3（#1 Galaxy S25 Ultra）续。**直连工具广度 6/6** 全真实数据：battery `71%`、storage `2.7GB used of 274.9GB`、wifi `enabled but not connected`、bluetooth `disabled`、notifications 读到真实系统通知、installed_apps `Found 14 apps`。**⭐ reliability 校验双向 E3（含 StackOverflow 修复路径）**：正例 `tap {x:100,y:200}` → `matchesType→asLong`（我修的递归 StackOverflow 那条路径）`ACTION_VALIDATION OK` + `Tapped at (100,200)`，**无崩溃/FATAL**；负例 `tap {}` → `ACTION_VALIDATION success=false INVALID_ACTION` + `ToolRegistry: Reliable action validation blocked 'tap': missing required parameter(s): x, y`，真实工具未执行。P0-1 reliability（抢救 + StackOverflow 修复）在真 arm64 设备双向验证完成。|
| 2026-07-08 | PASS | RELIABILITY-E3-CLOUDPHONE | **真机 E3**(无印云手机 #1 三星 Galaxy S25 Ultra / Android 15 / arm64）：服务器 build 的 `v0.7.2` debug APK 装上真机，授权无障碍+通知，`DEBUG_TASK config:` 配 DeepSeek（provider=OPENAI, model=deepseek-chat）成功。直连任务 `how much battery left` → `Pipeline Tier 1: DirectTool` → 真实 `Battery: 71%, not charging, 26.1°C` → `onComplete rounds=0 model=direct`。**⭐ reliability 层真机验证（E1→E3）**：logcat 完整 `startTask → ACTION_VALIDATION OK → ACTION_EXECUTION_START → ACTION_RESULT success=true errorType=NONE → finishTask events=5 actions=1 failed=0`。harness + reliability + 直连工具在真 arm64 设备上端到端通过。|
| 2026-07-08 | BLOCKED | CLOUDPHONE-DNS-CORE | 核心 agent-loop（DeepSeek 驱动）在 #1/#3 被 **DNS 阻断**：手机有外网（ping 8.8.8.8 通 ~100ms）但 resolver 空（解析不了任何域名，`Unable to resolve host "api.deepseek.com"`），无 root、private DNS（DoT dns.google）未救回。#2（Reno 10 Pro+）DNS 正常、能解析+ping 通 deepseek → 核心测试迁 #2（装机中）。**agent loop 机制本身已验证正常**（`Agent initialized provider=OPENAI model=deepseek-chat`、`pre-warm screen attached 1396 chars`、`onLoopStart round=1`、失败路径 reliability `finishTask status=failed` 均正确）——纯环境 DNS 问题，非 harness 缺陷。|
| 2026-07-07 | PASS | GUARD-COVERAGE-INAPPSEARCH | 自主迭代增量 E1：补 `InAppSearchGuardTest`（此前三个执行 Guard 中唯一无单测的；Email/DirectDeviceData 已有）。8 用例覆盖正向（`search youtube for X` / `search for X on youtube` 激活、input_text 精确/超集匹配解锁、无关文本不解锁、open_app 记录进度但仍需输入）+ 负向（普通对话 / 纯 open app / 空任务不激活、未匹配返回默认纠正）。正向用 common app（youtube 走 `OpenAppTool` 静态 map）保持 context-free 纯 JVM。`--tests InAppSearchGuardTest` → **8/8 通过**，BUILD SUCCESSFUL。|
| 2026-07-07 | BLOCKED | E2-EMULATOR-CEILING | 自主迭代基建判定：本沙箱（WSL2，内核 6.6-microsoft）**无 `/dev/kvm`**（有 vmx/svm 但内核未暴露 KVM），x86_64 Android emulator 无法硬件加速、软件渲染在无 KVM 下基本起不来 → **E2（emulator smoke）与 E3（real-device）在本环境不可用**。影响：P1-4 god-class UI 重构、P1-5/P1-6 等需回归验证的项在此**无法升过 E1**，一律标 E2-BLOCKED，建议在有 KVM 的宿主/真机上跑那批。本环境继续做 **E1 可完全验证** 的工作（纯逻辑 + 单测、文档）。|
| 2026-07-07 | PASS | P0-1-RELIABILITY-HARDEN | 自主迭代硬化 reliability（**E1**）——**发现并修复一个潜伏运行时 crash**：`ActionValidator` 私有扩展 `Any?.toLongOrNull()` 与 kotlin stdlib 同名，在其自身 `this?.toString()?.toLongOrNull()` 处递归调用自己 → `StackOverflowError`；dev 原 4 条测试从未覆盖 wait_after/整型校验路径故从未暴露（校验任何整型参数或 wait_after 的任务都会崩）。修复：改名 `asLong()` 破除遮蔽 + 加防复发注释；顺带清掉 `matchesType` dead-code。补 4 条 ActionValidator 边界单测（wait_after 越界/合法、类型不匹配/正确整型）+ `ExecutionTraceTest`。`:app:testDirectDebugUnitTest --tests "io.agents.pokeclaw.reliability.*"` → **9/9 通过**（0 fail），BUILD SUCCESSFUL in 44s。这批边界测试正是抓出该 crash 的原因。|
| 2026-07-07 | DOC | P1-3-CLOUD-BOUNDARY | 自主迭代 P1-3：新建 `CLOUD_SUBSYSTEM_BOUNDARY.md`，给 cloud/cloudnode/server/scheduler/role/job/memory（~90 文件）端云子系统划边界。据实核实门控——`initCloudNode()` 由 `cloud_enabled`（默认 false）门控 heartbeat/orchestrator 启动 + `CLOUD_WS_ENABLED=false`，「默认关闭」不变量**当前成立**（fresh install 无 enrollment/心跳/网络）。标注真问题：硬编码私有兜底 IP `192.168.250.3:8080`、disabled 仍 eager 构造、默认 env=PRODUCTION、从未真后端验证、reconstruction 计划未提。定义边界不变量并挂进 `AI_INDEX.md`；follow-up 入 BACKLOG。纯文档，无需构建。|
...
## Y. Agent-controls-Claw + Cloud-YOLO (per-software model center)

Cloud hub run once: `cd /workspace/dyq/claw-yolo-hub && ./run.sh` (→ `:8077`). App console:
`adb shell am start -n io.agents.pokeclaw/.ui.console.VisionConsoleActivity`, set Hub URL
(emulator → `http://10.0.2.2:8077`). E-levels per `## Evidence Levels`.

- [x] **Y1. Hub boots + seeds** (E-RUNTIME): start hub → `curl :8077/healthz` → `status=ok`, `software=4` (Claw/TikTok/Instagram/Chrome). Verified 2026-07-08.
- [x] **Y2. Routing tiers** (E1 unit `ModelRoutingTest` + E-RUNTIME curl): TikTok `package_name=com.zhiliaoapp.musically` → `source=category`; Claw `io.claw.app` → `source=generic_fallback needs_data=true`; unknown pkg auto-registers → generic. Candidate never routed as controlling (`ModelRoutePolicy`).
- [x] **Y3. Model download + checksum** (E-RUNTIME): resolve Claw → `GET /models/{id}/download` → bytes sha256 == descriptor `checksum` == `X-Model-Checksum`; generic serves real 22MB gxe weight.
- [x] **Y4. On-device model cache + version update** (E1 unit `ModelCacheTest` + `ModelRoutingTest`): store v1 → `cachedVersion`/`checksum` correct; `ModelUpdatePolicy` downloads when newer+idle, defers mid-task, no-op when up-to-date; checksum mismatch rejected.
- [x] **Y5. Weak-label detection from UI XML** (E1 unit `WeakLabelDetectorTest`): parse `getScreenTreeFull()` dump → nodes with role/fineRole/flags; normalized cxcywh boxes; invisible/zero-area dropped.
- [x] **Y6. Explore dedup + coverage + unexplored queue** (E1 unit `ExplorationEngineTest`): simulated app → unique states deduped by `StateHasher`, bounded by maxSteps, coverage per-activity, all real actions explored, frontier remains when stopped early.
- [x] **Y7. Sample payload + trajectory** (E1 unit `CollectPayloadTest`/`DataCollectorTest`): `SamplePayloadBuilder` matches hub `SampleIn` schema (session/step/pkg/activity/page_hash/boxes/action/model_id/screenshot/xml); trajectory JSON-lines.
- [x] **Y8. Cloud collect→train→promote→rollback** (E-RUNTIME `pytest tests/test_hub_e2e.py` 5/5): upload 6 samples by software_key → dataset visible in `/datasets` → `/training/{key}/trigger` → candidate (shadow-only in resolve) → gate `/promote` → `source=software_active` + `update_available` → 2nd train+promote → `/rollback` restores prior version; thin dataset blocked by gate, `force` overrides.
- [x] **Y9. Full APK builds** (E1): `:app:assembleDirectDebug` → APK with console registered in merged manifest.
- [ ] **Y10. On-device full loop** (E3, BLOCKED — no adb/device in sandbox): on a real cloud phone, open console → Load instances → Start exploration on a target app → logcat shows `SoftwareExplorer` resolve + per-page `collected` + hub dataset count rising → Trigger training → Promote → next explore resolves `software_active`. Cmd: `adb logcat --pid=$(pidof io.agents.pokeclaw) | grep -E "SoftwareExplorer|YoloModelClient|ModelHubClient|A11yObserver"`.
- [ ] **Y11. Remote cloud-phone control** (E3, BLOCKED): configure `cloudphone_base_url` to a live dyq → Load instances lists real instances → select → `CloudPhoneActuator.tap` drives `/admin-api/cloudphone/instance/control`. Needs a running dyq + vendor cloud phone.
- [ ] **Y12. Real YOLO inference** (BLOCKED — runtime not wired): once ONNX/LiteRT is in `YoloModelBackend`, boxes come from the downloaded weight (source=model) not weak labels; compare box IoU vs weak labels.
