...
- [x] ~~**P1** (H3 二级 cloud_agent 执行)~~ — 2026-07-09 **已实现(mvn 编译通过)**:独立 cloud-agent MQ 链路(`ClawCloudAgentTaskMessage`/`ClawCloudAgentRabbitMQConfig` 独立队列+DLQ/`ClawCloudAgentTaskConsumer`)+ `dispatchToCloud` 落库后投递 → 消费者用 **agent 独立身份**(`executorUserId`,不混真实用户,Nicole 要求)调 `Hermes.command`→`getOrCreateLobster(agentUserId)`→回写 `completeTask/failTask`。agent 身份经 `dyq.claw.cloud-agent.executor-user-id` 配置 + 入 `commandPayload` 可追溯。⚠️ 待 Hermes 环境端到端真验收。
- [ ] **P2** (H3 二级可追溯增强) `executorUserId` 目前入 `commandPayload`(JSON);Q1 提及可加独立 `claw_task.user_id` 列便于查询/统计,待补。
- [x] ~~**P1** 跨设备自进化上报(自进化跨设备汇聚,报告 row 9 STUB)~~ — 2026-07-09 **代码完成(未编译,用户要求直接提交)**:dyq 设备端点 `/api/claw-device/experience`(复用设备鉴权,写 `claw_experience` sourceDeviceId 维度)+ 端侧 `ExperienceUploader` 接线到 `CloudNodeOrchestrator` 云任务终态(此前从不被调用)。⚠️ 待编译+装包端到端真验收(装包→跑云任务→查 claw_experience)。
